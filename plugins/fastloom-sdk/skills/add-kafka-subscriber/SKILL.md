...
- **`KafkaSubscriber`'s constructor already defaults `ack_policy=NACK_ON_ERROR` broker-wide** — a subscriber whose handler raises gets exponential backoff (5s → 10s → ... capped at 30min, with jitter) for free. Only pass `ack_policy="ack_first"` on a specific `@subscriber(...)` call if that handler genuinely wants fire-and-forget semantics (no redelivery, no backoff) — that subscriber's exceptions just propagate without a retry.
- **The backoff sleep blocks that subscriber's whole poll loop** — every partition/topic it owns, not just the failing one. If other partitions need to keep flowing while one backs off, add `max_workers=N` to `@subscriber(...)` (FastStream processes up to N messages concurrently instead of one at a time — but then handlers for the same topic can run out of per-partition order, so only do this if the handler doesn't depend on strict ordering).
