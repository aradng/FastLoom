...
def rewrite_cache_meta(cls: type[BaseCache], **meta_fields: object) -> None:
    # NOTE: recurses into every subclass, however deep or however
    # late-defined — aredis_om snapshots Meta.global_key_prefix onto each
    # subclass at class-definition time rather than looking it up
    # dynamically, so a plain assignment on BaseCache only reaches
    # subclasses defined after this call, not ones already imported (e.g.
    # from a service's settings.py, which loads before Configs.__init__).
