...
- `fastloom.monitoring.Instruments` — enum of supported integrations (`REDIS`, `CELERY`, `RABBIT`, `HTTPX`, `REQUESTS`, `METRICS`, `MONGODB`, `PYDANTIC`, `PYDANTIC_AI`, `OPENAI`). Kafka is absent on purpose — it is traced by a FastStream middleware, see [Signals](signals.md#telemetry).
