...
`_dep` is a plain module-level function, not an instance method (`ContextSource._dep` is `staticmethod(_context_dep)`, kept for API consistency with the other sources) — it doesn't need `self` for anything, and referencing a closure variable like `self._dep` inside `Annotated[str, StreamDepends(self._dep)]` crashes AsyncAPI schema generation (`PydanticUserError: ... is not fully defined`), since that embeds an unresolvable forward reference only `fast_depends`'s own resolver can see, not FastStream's separate AsyncAPI schema builder. Referencing the module-level function directly avoids the whole class of problem.
