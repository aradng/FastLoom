...
`RabbitSubscriber` and `KafkaSubscriber` both construct **before** `get_app()`/`InitMonitoring` — `fastloom.launcher.utils.setup_brokers()` calls `instrument_brokers(TC.general)` first, then constructs whichever of the two subscribers apply. `instrument_brokers` runs `AioPikaInstrumentor` up front, independent of the rest of `InitMonitoring` (Sentry init, FastAPI instrumentation, service-specific `additional_instruments`), because it doesn't depend on data from `get_app()` — it's auto-inferred straight from `TC.general` (`infer_broker_instruments`).

Rabbit is the only broker `instrument_brokers` touches. Kafka's tracing is a FastStream broker middleware attached inside `get_kafka_router()` (see [Telemetry](#telemetry) below), so it carries no import-order constraint — the middleware resolves the tracer when a span starts, not when the module is imported.

The split used to exist for Kafka specifically: `ConfluentKafkaInstrumentor` patched `confluent_kafka.Producer`/`Consumer` by reassigning the *module attribute*, and FastStream's confluent client does `from confluent_kafka import Producer` at *import* time, so an import that ran before the patch pinned the unpatched class for the lifetime of the process. That instrumentor is gone and the constraint with it. `AioPikaInstrumentor` never had the problem — it wraps `Queue.consume`/`Exchange.publish` in place on the existing class objects, so it's insensitive to import order.

`kafka.depends` still defers its own `faststream.confluent` import into `get_kafka_router()`'s body, so a service that never installs the `kafka` extra can import the module eagerly. Constructing `KafkaSubscriber` outside the launcher needs no instrumentation call beforehand — pass settings carrying a truthy `OTEL_ENABLED` and the middleware attaches itself.
...
Kafka tracing is FastStream's own `KafkaTelemetryMiddleware`, attached by `get_kafka_router()` when the settings object is an `ObservabilitySettings` with a truthy `OTEL_ENABLED`. Settings without the flag get no middleware, so tracing is off rather than half-on. Spans are `<topic> publish` on the produce side and `<topic> process` on the consume side. A single message's consume span is **parented** to the producer's span through the W3C `traceparent` header; a batch is **linked** instead, since one span cannot have several parents.

`Instruments.KAFKA` and `instrument_confluent_kafka` no longer exist. The OTel contrib instrumentor attached the extracted context around `poll()`, which FastStream runs through `run_in_executor` on a single-worker `ThreadPoolExecutor` — and `run_in_executor` doesn't copy contextvars, so the handler, back on the event loop, never saw it. Every consumer started a fresh trace, and so did every database and Redis call inside the handler. It also parented the consume span with `links=` rather than `context=`. Upstream fixes this by not creating the poll span at all ([opentelemetry-python-contrib#5066](https://github.com/open-telemetry/opentelemetry-python-contrib/pull/5066)). Its instance-level `instrument_producer(producer)` remains the right tool for a raw `confluent_kafka` client that doesn't go through FastStream.

There's no Kafka equivalent of `RabbitPayloadTelemetryMiddleware` — that's payload-header enrichment, which is separate from the trace-context propagation `KafkaTelemetryMiddleware` already does.
